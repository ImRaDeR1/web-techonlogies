...
function task3() {
    const month = parseInt(prompt("Введите номер месяца (1-12):"), 10);
    const day = parseInt(prompt("Введите число в этом месяце (1-31):"), 10);
    ...
    const inputDate = new Date(2025, month - 1, day);
    
    // Проверка на корректность даты
    if (inputDate.getMonth() !== month - 1 || inputDate.getDate() !== day) {
        document.getElementById("result").textContent = "Ошибка: введена некорректная дата";
        return;
    }
    ...
    document.getElementById("result").textContent = `В этот день недели: ${dayOfWeek}`; // Исправлена строка
}
